import { thread, threadFromInt, type ThreadColor } from '../../pattern/thread.js';
import type { ThreadChart } from './palette-match.js';

/**
 * Machine thread charts.
 *
 * These are the colour tables built into the machines themselves, not
 * proprietary manufacturer colour cards — a PEC file stores "colour 5" and a
 * Brother machine displays *its* idea of colour 5. Reproducing the chart is
 * therefore a compatibility requirement, not a convenience.
 *
 * Index 0 is `null` in both charts: both formats reserve it, so valid indices
 * start at 1.
 */

function pec(r: number, g: number, b: number, description: string, code: string): ThreadColor {
  return thread(r, g, b, description, { catalogNumber: code, brand: 'Brother', chart: 'Brother' });
}

function jef(color: number, description: string, code: string): ThreadColor {
  const entry = threadFromInt(color, description);
  entry.catalogNumber = code;
  entry.brand = 'Janome';
  entry.chart = 'Jef';
  return entry;
}

/** Brother's 64-colour PEC chart, as used by the PE-series (including the PE900). */
export const BROTHER_PEC_THREADS: ThreadChart = Object.freeze([
  null,
...
/** Janome's JEF chart. */
export const JANOME_JEF_THREADS: ThreadChart = Object.freeze([
  null,
...
/** Chart entry for an index, wrapping like the machines do. Never `null`. */
export function chartThread(chart: ThreadChart, index: number): ThreadColor {
  const size = chart.length;
  const wrapped = ((index % size) + size) % size;
  return chart[wrapped] ?? chart[1] ?? thread(0, 0, 0, 'Black');
}
